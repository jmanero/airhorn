...
Mixer.prototype._next = function(callback) {
  ...
  var track = Spotify.Track.getFromUrl(this.tracks.shift());
...
/**
 * Add a track to the play queue. Load it if the player is idle
 */
Mixer.prototype.queue = function(tracks, callback) {
  if (!tracks) return callback(Error("No tracks"));
